...
@pytest.mark.parametrize(
    "user_id, word_language, word_in_progress, res", [(1, "en", 1, ["test1"])]
)
def test_fetch_words_for_repetition(
    ...
    words = word_repo.fetch_words_for_repetition(
        user_id, word_language, word_in_progress
    )
    assert [word.word for word in words] == res


@pytest.mark.parametrize(
    "user_id, word_language, word_in_progress, res", [(2, "en", 2, None)]
)
