...
def start_local_service():
    with open(
        f"{os.path.abspath(os.curdir)}/bot/scripts/sql/local_service.sql"
    ) as sql_file:
        text = sql_file.read()
        commands = text.split("\n")
    ...
    progress_points = [
        ...
        int(commands_count / 4 * 2),
        int(commands_count / 4 * 3),
        commands_count - 1,
    ]
    try:
            ...
            with conn.cursor() as cur:
                for index, command in enumerate(commands):
                    ...
                    if index in progress_points:
                        ...
                        logger.info(
                            f"{db_host}:{db_port} - Service is {int(percent)}% complete"
                        )
                    time.sleep(0.3)
