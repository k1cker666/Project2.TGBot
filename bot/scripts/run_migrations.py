...
def create_database():
    try:
        with psycopg.connect(
            ...
            logger.info(
                f"{db_host}:{db_port} - Connection to PostgreSQL as user {db_user} successful"
            )
            with conn.cursor() as cur:
                cur.execute(
                    "select 1 from pg_database where datname = 'tgbot';"
                )
                result = cur.fetchone()
                if result is None:
                    cur.execute("create database tgbot;")
                    logger.info(
                        f"{db_host}:{db_port} - Database {db_name} created"
                    )
                else:
                    logger.info(
                        f"{db_host}:{db_port} - Database {db_name} already exists"
                    )
            ...
            with conn.cursor() as cur:
                for file in migrations:
                    ...
                    with open(path, "r") as command:
                        ...
                        logger.info(
                            f"{db_host}:{db_port} - Migration {file} complete"
                        )
    except Exception as error:
        logger.error(f"{db_host}:{db_port} - {error}")
